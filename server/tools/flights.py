...
MOCK_CMD = [sys.executable, str(Path(__file__).resolve().parents[2] / "mocks" / "flights_mcp.py")]
...
def _parse_tool_result(result) -> List[dict]:
    if result.structuredContent is not None:
        ...
        if isinstance(structured, dict):
            for key in ("items", "data", "result", "flights"):
...
async def search_flights(origin: str, destination: str, date: str, passengers: int) -> List[dict]:
