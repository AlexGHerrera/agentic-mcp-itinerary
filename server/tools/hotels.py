...
MOCK_CMD = [sys.executable, str(Path(__file__).resolve().parents[2] / "mocks" / "hotels_mcp.py")]
...
def _parse_tool_result(result) -> List[dict]:
    if result.structuredContent is not None:
        ...
        if isinstance(structured, dict):
            for key in ("items", "data", "result", "hotels"):
...
async def search_hotels(city: str, checkin: str, checkout: str, guests: int) -> List[dict]:
